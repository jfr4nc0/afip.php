<?php
/**
 * SDK for AFIP Electronic Billing (wsmtxca)
 * 
...
class ElectronicBilling_Details extends AfipWebService {
	...
	public function GetLastVoucher($sales_point, $type)
	{
		...
		$res = $this->ExecuteRequest('consultarUltimoComprobanteAutorizado', $req)->numeroComprobante;
		return $res;
	}
	...
	**/
	...
	{
		$req = array(
			    'comprobanteCAERequest' => &$data
			);

		if (isset($data['arrayOtrosTributos'])) 
			$data['arrayOtrosTributos'] = array('otroTributo' => $data['arrayOtrosTributos']);

		if (isset($data['arrayItems']))
		    $data['arrayItems'] = array('item' => $data['arrayItems']);
			
		if (isset($data['arraySubtotalesIVA'])) 
			$data['arraySubtotalesIVA'] = array('subtotalIVA' => $data['arraySubtotalesIVA']);

		if (isset($data['arrayDatosAdicionales']))
		    $data['arrayDatosAdicionales'] = array('datoAdicional' => $data['arrayDatosAdicionales']);
			
		if (isset($data['arrayCompradores'])) 
			$data['arrayCompradores'] = array('comprador' => $data['arrayCompradores']);

		if (isset($data['arrayActividades']))
		    $data['arrayActividades'] = array('actividad' => $data['arrayActividades']);
		...
		if ($return_response === TRUE) {
			...
			return array(
				...
				'fechaVencimientoCAE' => $results->comprobanteResponse->fechaVencimientoCAE,
			);
		}
	}
	...
	**/
	...
	{
		$req = array(
			'consultaComprobanteRequest' => array(
				'codigoTipoComprobante' => $type,
				'numeroPuntoVenta' 	=> $sales_point,
	...
	**/
	...
	{
	    $res = $this->ExecuteRequest('consultarPuntosVentaCAE');
	...
	**/
	...
	{
	    $res = $this->ExecuteRequest('consultarTiposComprobante');
		return get_object_vars($res);
	}
...
// 	}
	...
	**/
	...
	{
		$res = $this->ExecuteRequest('consultarTiposDocumento');
		return get_object_vars($res);
	}
	...
	{
	    $res = $this->ExecuteRequest('consultarActividadesVigentes');
	    return get_object_vars($res);
	}
	...
	**/
	...
	{
		$res = $this->ExecuteRequest('consultarAlicuotasIVA');
		return get_object_vars($res);
	}
	...
	**/
	...
	{
		$res = $this->ExecuteRequest('consultarMonedas');
		return get_object_vars($res);
	}
...
// 	}
	...
	**/
	...
	{
		$res = $this->ExecuteRequest('consultarTiposTributo');
		return get_object_vars($res);
	}
	...
	**/
	...
	{
		$this->options = array('service' => 'wsmtxca');
	...
	private function _CheckErrors($operation, $results)
	{
		...
		if ($operation == 'autorizarComprobanteRequest') {
			if (isset($res->arrayObservaciones->codigoDescripcion)) {
				$res->Errors = new StdClass();
				$res->Errors->Err = $res->arrayObservaciones->codigoDescripcion;
			}
		}

		if (isset($res->arrayErrores)) {
		    $err = is_array($res->arrayErrores->codigoDescripcion) ? $res->arrayErrores->codigoDescripcion[0] : $res->arrayErrores->codigoDescripcion;
